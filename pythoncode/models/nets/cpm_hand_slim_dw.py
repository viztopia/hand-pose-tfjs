    ...
    def build_model(self, input_image, center_map, batch_size):
        ...
        with slim.arg_scope([slim.separable_conv2d],
            ...
            with tf.variable_scope('sub_stages'):
                ...
                net = slim.separable_conv2d(net, 512, [3, 3], scope='sub_conv9')
                net = slim.separable_conv2d(net, 512, [3, 3], scope='sub_conv10')
                net = slim.separable_conv2d(net, 512, [3, 3], scope='sub_conv11')
                net = slim.separable_conv2d(net, 512, [3, 3], scope='sub_conv12')
                net = slim.separable_conv2d(net, 512, [3, 3], scope='sub_conv13')
                net = slim.separable_conv2d(net, 512, [3, 3], scope='sub_conv14')

                self.sub_stage_img_feature = slim.separable_conv2d(net, 128, [3, 3],
                                                         scope='sub_stage_img_feature')

            with tf.variable_scope('stage_1'):
                conv1 = slim.separable_conv2d(self.sub_stage_img_feature, 512, [1, 1],
                                    scope='conv1')
                self.stage_heatmap.append(slim.separable_conv2d(conv1, self.joints, [1, 1],
                                                      scope='stage_heatmap'))
    ...
    def _middle_conv(self, stage):
        with tf.variable_scope('stage_' + str(stage)):
            ...
            with slim.arg_scope([slim.separable_conv2d],
                                ...
                                activation_fn=tf.nn.leaky_relu,
                                weights_initializer=tf.contrib.layers.xavier_initializer()):
                mid_net = slim.separable_conv2d(self.current_featuremap, 128, [7, 7], scope='mid_conv1')
                mid_net = slim.separable_conv2d(mid_net, 128, [7, 7], scope='mid_conv2')
                mid_net = slim.separable_conv2d(mid_net, 128, [7, 7], scope='mid_conv3')
                mid_net = slim.separable_conv2d(mid_net, 128, [7, 7], scope='mid_conv4')
                mid_net = slim.separable_conv2d(mid_net, 128, [7, 7], scope='mid_conv5')
                mid_net = slim.separable_conv2d(mid_net, 128, [1, 1], scope='mid_conv6')
                self.current_heatmap = slim.separable_conv2d(mid_net, self.joints, [1, 1],
                                                   scope='mid_conv7')
