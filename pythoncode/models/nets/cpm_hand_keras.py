...
class CPM_Model(object):
    ...
    def build_model_mobilenet(self, input_image, center_map, batch_size):
        inputs = Input(shape=(368, 368, 3))
        ...
        net = Conv2D(kernel_size=3, strides=1, filters=256, padding='same',
                     activation='relu', name='sub_stages/sub_conv5')(net)
        # net = SeparableConv2D(kernel_size=3, strides=1, filters=256, padding='same', activation='relu',
        #                       name='sub_stages/sub_conv5')(net)
        net = Conv2D(kernel_size=3, strides=1, filters=256, padding='same',
                     activation='relu', name='sub_stages/sub_conv6')(net)
        # net = SeparableConv2D(kernel_size=3, strides=1, filters=256, padding='same', activation='relu',
        #                       name='sub_stages/sub_conv6')(net)
        net = Conv2D(kernel_size=3, strides=1, filters=256, padding='same',
                     activation='relu', name='sub_stages/sub_conv7')(net)
        # net = SeparableConv2D(kernel_size=3, strides=1, filters=256, padding='same', activation='relu',
        #                       name='sub_stages/sub_conv7')(net)
        net = Conv2D(kernel_size=3, strides=1, filters=256, padding='same',
                     activation='relu', name='sub_stages/sub_conv8')(net)
        net = MaxPooling2D(pool_size=2, name='sub_stages/sub_pool3')(net)
        # net = SeparableConv2D(kernel_size=3, strides=2, filters=256, padding='same', activation='relu',
        #                       name='sub_stages/sub_conv8')(net)

        # net = Conv2D(kernel_size=3, strides=1, filters=512, padding='same',
        #              activation='relu', name='sub_stages/sub_conv9')(net)
        net = SeparableConv2D(kernel_size=3, strides=1, filters=512, padding='same', activation='relu',
                              name='sub_stages/sub_conv9')(net)
        # net = Conv2D(kernel_size=3, strides=1, filters=512, padding='same',
        #              activation='relu', name='sub_stages/sub_conv10')(net)
        net = SeparableConv2D(kernel_size=3, strides=1, filters=512, padding='same', activation='relu',
                              name='sub_stages/sub_conv10')(net)
        # net = Conv2D(kernel_size=3, strides=1, filters=512, padding='same',
        #              activation='relu', name='sub_stages/sub_conv11')(net)
        net = SeparableConv2D(kernel_size=3, strides=1, filters=512, padding='same', activation='relu',
                              name='sub_stages/sub_conv11')(net)
        # net = Conv2D(kernel_size=3, strides=1, filters=512, padding='same',
        #              activation='relu', name='sub_stages/sub_conv12')(net)
        net = SeparableConv2D(kernel_size=3, strides=1, filters=512, padding='same', activation='relu',
                              name='sub_stages/sub_conv12')(net)
        # net = Conv2D(kernel_size=3, strides=1, filters=512, padding='same',
        #              activation='relu', name='sub_stages/sub_conv13')(net)
        net = SeparableConv2D(kernel_size=3, strides=1, filters=512, padding='same', activation='relu',
                              name='sub_stages/sub_conv13')(net)
        # net = Conv2D(kernel_size=3, strides=1, filters=512, padding='same',
        #              activation='relu', name='sub_stages/sub_conv14')(net)
        net = SeparableConv2D(kernel_size=3, strides=1, filters=512, padding='same', activation='relu',
                              name='sub_stages/sub_conv14')(net)
        # net = Conv2D(kernel_size=3, strides=1, filters=128, padding='same',
        #              activation='relu', name='sub_stages/sub_stage_img_feature')(net)
        net = SeparableConv2D(kernel_size=3, strides=1, filters=128, padding='same', activation='relu',
                              name='sub_stages/sub_stage_img_feature')(net)

        self.sub_stage_img_feature = net

        net = SeparableConv2D(kernel_size=1, strides=1, filters=512, padding='same',
                     activation='relu', name='stage_1/conv1')(net)
        net = SeparableConv2D(kernel_size=1, strides=1, filters=self.joints, padding='same',
                     activation='relu', name='stage_1/stage_heatmap')(net)
        ...
        for stage in range(2, self.stages + 1):
            self.current_featuremap = concatenate([self.stage_heatmap[stage - 2],
                                                  ...
                                                  axis=3)
            net = SeparableConv2D(kernel_size=7, strides=1, filters=128, padding='same',
                         activation='relu', name='stage_' + str(stage) + '/mid_conv1')(self.current_featuremap)
            net = SeparableConv2D(kernel_size=7, strides=1, filters=128, padding='same',
                         activation='relu', name='stage_' + str(stage) + '/mid_conv2')(net)
            net = SeparableConv2D(kernel_size=7, strides=1, filters=128, padding='same',
            ...
            net = SeparableConv2D(kernel_size=7, strides=1, filters=128, padding='same',
                         activation='relu', name='stage_' + str(stage) + '/mid_conv5')(net)
            net = SeparableConv2D(kernel_size=1, strides=1, filters=128, padding='same',
                         activation='relu', name='stage_' + str(stage) + '/mid_conv6')(net)
            self.current_heatmap = SeparableConv2D(kernel_size=1, strides=1, filters=self.joints, padding='same',
                                          activation='relu', name='stage_' + str(stage) + '/mid_conv7')(net)
            self.stage_heatmap.append(self.current_heatmap)
            # outputs = self.current_heatmap
    ...
    def set_weights_mobilenet(self, model_vars):
        model = self.model
        ...
        for i in range(9, 15):
            layer = model.get_layer(name='sub_stages/sub_conv' + str(i))
            layer.set_weights([model_vars['sub_stages/sub_conv' + str(i) + '/depthwise_weights:0'],
                               model_vars['sub_stages/sub_conv' + str(i) + '/pointwise_weights:0'],
                               model_vars['sub_stages/sub_conv' + str(i) + '/biases:0']])

        layer = model.get_layer(name='sub_stages/sub_stage_img_feature')
        layer.set_weights([model_vars['sub_stages/sub_stage_img_feature/depthwise_weights:0'],
                           model_vars['sub_stages/sub_stage_img_feature/pointwise_weights:0'],
                           model_vars['sub_stages/sub_stage_img_feature/biases:0']])

        layer = model.get_layer(name='stage_1/conv1')
        layer.set_weights([model_vars['stage_1/conv1/depthwise_weights:0'],
                           model_vars['stage_1/conv1/pointwise_weights:0'],
                           model_vars['stage_1/conv1/biases:0']])

        layer = model.get_layer(name='stage_1/stage_heatmap')
        layer.set_weights([model_vars['stage_1/stage_heatmap/depthwise_weights:0'],
                           model_vars['stage_1/stage_heatmap/pointwise_weights:0'],
                           model_vars['stage_1/stage_heatmap/biases:0']])

        for i in range(2, 7):
            for j in range(1, 8):
                layer = model.get_layer(name='stage_' + str(i) + '/mid_conv' + str(j))
                layer.set_weights([model_vars['stage_' + str(i) + '/mid_conv' + str(j) + '/depthwise_weights:0'],
                                   model_vars['stage_' + str(i) + '/mid_conv' + str(j) + '/pointwise_weights:0'],
                                   model_vars['stage_' + str(i) + '/mid_conv' + str(j) + '/biases:0']])
